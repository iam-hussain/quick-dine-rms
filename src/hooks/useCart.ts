...
function useCart({ control, order }: UseCartType) {
  ...
  return {
    ...
    packagingCharge,
    type,
    allItems: (order?.items || []) as any[],
    scheduledItems: (order?.scheduled || []) as any[],
    placedItems: (order?.placed || []) as any[],
    acceptedItems: (order?.accepted || []) as any[],
    preparedItems: (order?.prepared || []) as any[],
    items: currentItems,
    total,
    taxesValue,
    grandTotal,
